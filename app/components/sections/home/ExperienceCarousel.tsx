          ...
          >
            {SLIDES.map((slide, i) => (
              <button
                ...
                onClick={() => navigate(i)}
                className={`relative shrink-0 px-4 md:px-5 py-2.5 font-serif font-bold text-base md:text-lg whitespace-nowrap transition-colors duration-200 ${
                  i === active
                    ? 'text-foreground'
                    : 'text-muted-foreground/40 hover:text-muted-foreground/70'
                }`}
              >
                <span className="relative z-10">{renderWithAnimatedVuew(slide.label)}</span>
              </button>
            ))}
          </div>
      ...
      <div className="relative overflow-hidden">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            animate={{ x: 0, opacity: 1, transition: { duration: 0.52, ease: 'easeOut' } }}
            exit={{ x: dir > 0 ? '-28%' : '28%', opacity: 0, transition: { duration: 0.3, ease: 'easeIn' } }}
            drag="x"
        ...
        <button
          ...
          className="hidden md:flex absolute left-4 top-1/2 -translate-y-1/2 z-10 w-14 h-14 items-center justify-center rounded-full border border-border/40 bg-background/60 backdrop-blur-sm text-muted-foreground/60 hover:text-foreground hover:border-border hover:bg-background/80 transition-all duration-200 shadow-sm"
        >
        ...
        <button
          ...
          className="hidden md:flex absolute right-4 top-1/2 -translate-y-1/2 z-10 w-14 h-14 items-center justify-center rounded-full border border-border/40 bg-background/60 backdrop-blur-sm text-muted-foreground/60 hover:text-foreground hover:border-border hover:bg-background/80 transition-all duration-200 shadow-sm"
        >
