...
export function StoreButton({ store, href = "#" }: { store: 'apple' | 'google', href?: string }) {
  return (
    <a 
      href={href}
      className="inline-flex items-center justify-center gap-2 bg-black text-white px-4 py-2 rounded-full hover:bg-black/90 transition-colors border border-black/10"
    >
      {store === 'apple' ? (
      ...
      <div className="flex flex-col items-start leading-none">
        <span className="text-[10px] opacity-80">{store === 'apple' ? 'Download on the' : 'GET IT ON'}</span>
        <span className="text-sm font-semibold font-sans">{store === 'apple' ? 'App Store' : 'Google Play'}</span>
      </div>
    </a>
  );
}
